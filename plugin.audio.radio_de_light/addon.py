import sys, xbmcaddon, os, requests, xbmc, xbmcgui, urllib.request, urllib.parse, urllib.error, re, xbmcplugin, html, json, xbmcvfs
...
def addLink(link, name, image, desc, urlType, fanart):
    ...
    contextMenuItems = []
    contextMenuItems.append((L(30007), f'RunPlugin(plugin://plugin.audio.radio_de_light/{add})'))
...
def addMy(link, name, image, desc, urlType, fanart):
    ...
    liz.setArt({'icon': image, 'thumb': image, 'poster': image, 'fanart': addonfanart})
    contextMenuItems = []
    contextMenuItems.append((L(30008), f'RunPlugin(plugin://plugin.audio.radio_de_light/{rem})'))
    liz.addContextMenuItems(contextMenuItems, replaceItems=True)
    ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=url, listitem=liz)
